...
blogsRouter.post('/', async (request, response) => {
  ...
  blogsRouter.delete('/:id', async (request, response) => {
    ...
    const token = authorizaiton.replace('Bearer ', '')
    if(!authorizaiton && !authorizaiton.startsWith('Bearer ')) {
      return response.status(401).json({error: 'Unauthorized or Try signing in again. '})
    }
    const decodedToken = jwt.verify(token, process.env.SECRET)
    ...
    if(!decodedToken.id || !token || decodedToken.id.toString() !== blog.user.toString()) {
  ...
  blogsRouter.put('/:id', async (request, response) => {
    ...
    const updatedBlog = await Blog.findByIdAndUpdate(request.params.id, blog, {new: 'ture'}).populate('user', {username:1, name:1})
